...
class AdvancedAdminViewModel(
    ...
    fun loadConfig(page: String) {
        _uiState.update { it.copy(isLoading = true, loadProgress = 0.2f, loadStatus = "Fetching Administration settings...", errorMessage = null, saveSuccess = false) }
        viewModelScope.launch {
            repository.getAdminConfig(page).collect { result ->
                result.onSuccess { config ->
                    _uiState.update { it.copy(config = config, loadProgress = 1.0f, loadStatus = "Administration settings loaded.", isLoading = false) }
                }
